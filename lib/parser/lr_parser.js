...
function shift(token) {
	...
	if(!action) {
		var message = "Parse error near '" + token.value + "'";
		if(token === EOF_symbol) {
			message = "Parse error at end of file.";
		}

		this.emit("parse_error", {"message":message, "token":token, "token_stack":token_stack});

		console.log(token_stack, this.getParserDescription().actionTable[current_state]);
	}
