 ...
 **/
var NonTerminalPrototype	= require("../tokenizer/token.js").NonTerminalPrototype;
 ...
 * Parser Prototype
 **/
var LRParserPrototype = {

	// Exposed functions
	"shift":shift,
	"end":function() { return this.shift(EOF_symbol); },
	...
	"EOF":EOF_symbol,
	"EPSILON":epsilon,
	"START_SYMBOL":augmentation_symbol
};
 ...
 **/
function CreateLRParser(parserDescription, acceptCallback) {
	...
	parser.acceptCallback = acceptCallback;
...
function shift(token) {
	var type = token.getType ? token.getType() : token;
	...
	if(action.operation === "shift") {
		console.debug("[Parser]["+current_state+"] Shift: ", token.getType());
		...
		for(var i in production.body) {
			...
			if(!this.getParserDescription().symbols[tok.value] || this.getParserDescription().symbols[tok.value].exclude !== true) {
				params.unshift(tok);
			}
		}
		...
		this.token_stack.push(NonTerminalPrototype.create(production.head, params));
		...
		if(this.acceptCallback) {
			this.acceptCallback(this.token_stack);
		}
	} else {
...
module.exports.factory = CreateLRParser;